 ...
 */
public class AccessStampMapping extends ElasticMapping {
    ...
    public static EFieldType makeEFieldType(Class c, Field f) {
        ...
        if (fieldname.matches("eUser")) {
            Class c1 = MyHelpMethods.getClassFromListOfClasses(f);
            EObjectType type = new EObjectType();
            type.properties=ElasticUserMapping.mapFieldsFromClass(c1);
            return type;
        } else if (fieldname.matches("name")) {
